                                  ...
                                  s32_t timeout)
{
    ...
    printf("rc=%d", pool->free_count);
#endif /* ADV_NET_BUF_DEBUG_EN */
    key = irq_lock();
    ...
    if (pool->free_count) {
        ...
        } while (buf->flags | BT_MESH_ADV(buf)->busy);
...
u32 get_list_count(sys_slist_t *list)
{
    u32 cnt = 0;
    sys_snode_t *node = list->head;
    while (node) {
